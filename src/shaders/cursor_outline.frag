...
layout(set=1, binding=0) uniform sampler depth_sampler;
layout(set=1, binding=1) uniform texture2D depth_texture;

#define DEPTH sampler2D(depth_texture, depth_sampler)
...
void main() {
    ...
    if (depth == 0.0) {
        f_color = vec4(0.0, 0.0, 1.0, alpha);
    } else {
        discard;
    }
}
